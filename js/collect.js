	...
	ajaxObj.collectRecords(
		function(){
			if(hasData){
				...
				s='<div style="height:60px;color: #cccccc;font-size: 28px;text-align: center;padding-top: 200px;">您还没收藏记录，快去收藏节目吧！</div>';
...
var contentPad={
	...
	renderHtml : function(){
		...
		if(this.currentPageData.length<=0){
			s='<div style="height:60px;color: #cccccc;font-size: 20px;text-align: center;padding-top: 200px;">您还没有收藏记录，快去收藏节目吧！</div>';
		}
...
var ajaxObj = {
	...
	collectRecords: function (_fn,_pageSize,_pageNo){
		ajaxObj.ajaxGo(
			serverPath+"/media/video/collectRecords.utvgo?keyNo="+keyNo+"&pageSize="+_pageSize+"&pageNo="+_pageNo,
			function(_data){
...
function generatePlayPageUrl(index){
	...
	if(contentPad.currentPageData[index].contentType==0){
		var url='play_audio.html?multiSetType='+multiSetType+'&channelId='+channelId+'&pkId='+pkId;
	}else if(contentPad.currentPageData[index].contentType==1){
		var url='detail.html?multiSetType='+multiSetType+'&channelId='+channelId+'&pkId='+pkId;
	}
	return url;
}

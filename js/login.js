...
var ajaxObj = {
	...
	newSession: function (_fn){
		ajaxObj.ajaxGo(
			serverPath+"/doll/dollForwardController/newSession.utvgo",
			function(_data){
				$('newSession_img').src = _data.qrcodeUrl;
				clientId = _data.session.clientId;
				ticket = _data.session.ticket;
				!!_fn&&_fn();
			},
			function(_data){
				clearInterval(timer1);
	...
	changeCode: function (_fn){
		ajaxObj.ajaxGo(
			serverPath+"/doll/dollForwardController/changecode.utvgo?clientId="+clientId,
			function(_data){
				$('newSession_img').src = _data.qrcodeUrl;
				clientId = _data.session.clientId;
				ticket = _data.session.ticket;
	...
	pollResult: function (_fn){
		ajaxObj.ajaxGo2(
			serverPath+"/doll/dollForwardController/pollresult.utvgo?keyNo="+keyNo+"&clientId="+clientId+"&ticket="+ticket,
			function(_data){
				if(_data.action=="confirm"){
					clearInterval(timer1);
					timer1=1;
	...
	userInfo: function (_fn){
		ajaxObj.ajaxGo2(
			serverPath+"/doll/dollForwardController/userInfo.utvgo?keyNo="+keyNo+"&token="+token,
			function(_data){
				showTips('绑定成功!',2000);
				!!_fn&&_fn();
			},

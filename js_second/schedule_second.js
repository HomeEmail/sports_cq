			...
			for(var j=0;j<this.colNum;j++){
				...
				item=[
					'<div id="'+this.itemId+'_'+(dataIndex)+'" style="position:absolute;z-index:2;top:'+(this.initTop+(this.itemHeight*i)+(i*this.rowMargin))+'px;left:223px;width:990px;height:90px;background: url(images_second/schedule_bg.png) no-repeat;display:block;">',
						...
						'<div style="position: absolute;height: 90px;line-height: 90px;color:#bfc1cd;font-size: 20px;width: '+colTitleAry[1].w+'px;left:'+colTitleAry[1].left+'px;top: 0px;text-align: left;z-index: 3;overflow: hidden;"><div style="padding-left:0px; padding-right: 10px;">'+(this.currentPageData[dataIndex].startTime)+'</div></div>',
						...
						'<div style="position: absolute;height: 90px;line-height: 90px;color: '+(this.currentPageData[dataIndex].status==-1?'#bfc1cd':(this.currentPageData[dataIndex].status==0?'#44c711':'#1c8cff'))+';font-size: 20px;width: '+colTitleAry[7].w+'px;left:'+colTitleAry[7].left+'px;top: 0px;text-align: center;z-index: 3;overflow: hidden;"><div style="padding-left:10px;padding-right:10px;">'+(this.currentPageData[dataIndex].status==-1?'未开始':(this.currentPageData[dataIndex].status==0?'比赛中':'比赛结束'))+'</div></div>',
